...
public class GammaArrayStats7s extends BaseShipSystemScript {

	public static String GAMMA_WINDUP = "gamma_array_windup7s";
	
	public static Color WEAPON_GLOW = new Color(70, 255, 156,155);
	

	
	
	public static class GammaDishData {
	...
	protected List<GammaDishData> dishData = new ArrayList<GammaArrayStats7s.GammaDishData>();
	...
	public void init(ShipAPI ship) {
		...
		for (WeaponAPI w : ship.getAllWeapons()) {
			if (w.isDecorative() && w.getSpec().hasTag("gamma")) {
				count++;
			}
		}
		List<WeaponAPI> gamma = new ArrayList<WeaponAPI>();
		for (WeaponAPI w : ship.getAllWeapons()) {
			if (w.isDecorative() && w.getSpec().hasTag("gamma")) {
				gamma.add(w);
			}
		}
		Collections.sort(gamma, new Comparator<WeaponAPI>() {
			public int compare(WeaponAPI o1, WeaponAPI o2) {
				return (int) Math.signum(o1.getSlot().getLocation().x - o2.getSlot().getLocation().x);
			}
		});
		for (WeaponAPI w : gamma) {
			if (w.isDecorative() && w.getSpec().hasTag("gamma")) {
				w.setSuspendAutomaticTurning(true);
				GammaDishData data = new GammaDishData();
	...
	public void rotateGammaDishes(boolean active, float effectLevel) {
		...
		//turnRateMult = 0.1f;
		//boolean first = true;
		for (GammaDishData data : dishData) {
...
//			}
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		rotateGammaDishes(active, effectLevel);
		...
		float GammaRange = 500;
		for (WeaponAPI w : ship.getAllWeapons()) {
			if (!w.isDecorative() && w.getSlot().isHardpoint() && !w.isBeam() &&
					(w.getType() == WeaponType.BALLISTIC || w.getType() == WeaponType.ENERGY)) {
				GammaRange = Math.max(GammaRange, w.getRange());
				w.setGlowAmount(effectLevel, glowColor);
			}
		}
		GammaRange += 100f;
		stats.getBeamWeaponRangeBonus().modifyFlat("gamma_array", GammaRange);
//
		...
		for (GammaDishData data : dishData) {
			...
			skip = false;
			if (skip) continue;
			if (data.w.isDecorative() && data.w.getSpec().hasTag("gamma")) {
		...
		if (((state == State.IN && effectLevel > 0.2f) || state == State.ACTIVE) && !playedWindup) {
			Global.getSoundPlayer().playSound(GAMMA_WINDUP, 1f, 1f, ship.getLocation(), ship.getVelocity());
			playedWindup = true;
		}
	}
	...
	protected void modify(String id, MutableShipStatsAPI stats, float effectLevel) {
		stats.getBallisticWeaponRangeBonus().modifyPercent(id, 100);
		stats.getEnergyWeaponRangeBonus().modifyPercent(id, 100);

		stats.getBallisticRoFMult().modifyMult(id, 0.6f);
		stats.getEnergyRoFMult().modifyMult(id, 0.6f);
		stats.getBallisticAmmoBonus().modifyMult(id,0.6f);
		stats.getEnergyAmmoRegenMult().modifyMult(id,0.6f);
		stats.getBallisticWeaponFluxCostMod().modifyMult(id,3f);
		stats.getEnergyWeaponFluxCostMod().modifyMult(id,3f);

		stats.getBallisticWeaponDamageMult().modifyMult(id, 3f);
		stats.getEnergyWeaponDamageMult().modifyMult(id,3f);
		//stats.getBallisticWeaponFluxCostMod().modifyMult(id, 1f - (FLUX_REDUCTION * 0.01f));
		stats.getMaxRecoilMult().modifyMult(id, 1f - (0.01f * 90f));
		stats.getRecoilPerShotMult().modifyMult(id, 1f - (0.01f * 90f));
		stats.getRecoilDecayMult().modifyMult(id, 1f - (0.01f * 90f));
		
		stats.getBallisticProjectileSpeedMult().modifyPercent(id, 100f);
		stats.getEnergyProjectileSpeedMult().modifyPercent(id, 100f);
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		if (state == State.IDLE || state == State.COOLDOWN) {
			if (index == 3) {
				return new StatusData("ship max speed +" + 30, false);
			}	
		}
		if (effectLevel <= 0f) return null;

		if (index == 5) {
			return new StatusData("weapon damage +" + 200 + "%", false);
		}

		if (index == 4) {
			return new StatusData("weapon range +" + 100 + "%", false);
		}

		if (index == 3) {
			return new StatusData("weapon flux cost +" + 200 + "%", true);
		}

		if (index == 2) {
			return new StatusData("rate of fire -" + 40 + "%", true);
		}
//		if (index == 1) {
//			return new StatusData("ballistic flux use -" + (int) FLUX_REDUCTION + "%", false);
//		}
		if (index == 1) {
			return new StatusData("weapon recoil -" + (int) 90 + "%", false);
		}
		if (index == 0 && 100 > 0) {
			return new StatusData("projectile speed +" + (int) 100 + "%", false);
		}
		return null;
	}
