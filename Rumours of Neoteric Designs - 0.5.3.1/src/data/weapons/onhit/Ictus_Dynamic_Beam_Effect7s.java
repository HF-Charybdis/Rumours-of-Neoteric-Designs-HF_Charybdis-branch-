...
public class Ictus_Dynamic_Beam_Effect7s implements BeamEffectPlugin {
    ...
    public void advance(float amount, CombatEngineAPI engine, BeamAPI beam) {
        ...
        float time = Global.getCombatEngine().getTimeMult().getMult();
        int alpha_value1 = Math.round(beam.getBrightness() * 50f * time);
        ...
        if (beam.getBrightness() > 0f) {
            MagicRender.battlespace(Global.getSettings().getSprite("graphics/starscape/star1.png"), beam.getFrom(), new Vector2f(),
                    ...
                    0f,
            new Color(154, 255, 189, alpha_value1),
                    true,
                    0.07f,
                    0f,
                    0.14f);
        ...
        if (target instanceof ShipAPI && beam.getBrightness() >= 1f) {
            ...
            new Color(154, 255, 189);

            float power = 0;
            if (MathUtils.getDistance(target,weapon.getShip()) >= 125f) {
                power = 20 - (weapon.getShip().getHardFluxLevel() * 40);
            }
            else if (MathUtils.getDistance(target,weapon.getShip()) < 125f) {
                power = -10;
            }

            float force = 1f * (((power) * (1f + (time * 0.2f))) - (0.01f * (MathUtils.getDistance(weapon.getLocation(), target.getLocation()))));
            if (force < 0 && weapon.getShip().getHardFluxLevel() < 0.5f) force = 0;

            CombatUtils.applyForce(target, VectorUtils.getAngle(target.getLocation(), weapon.getLocation()), force);

            if (Global.getCombatEngine().getPlayerShip() == target && weapon.getShip().getHardFluxLevel() < 0.5f) {
