...
public class ElectricDispenserOnFire7s implements OnFireEffectPlugin {
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		...
		if (target != null) {
			EmpArcEntityAPI arc = engine.spawnEmpArc(projectile.getSource(), projectile.getLocation(), weapon.getShip(),
					   ...
					   emp, // emp
					   100000f, // max range
					   "Electric_dispenser7s_fire",
					   thickness, // thickness
					   color,
					   new Color(255, 205, 205,255)
					   );
			//Global.getSoundPlayer().playSound("mote_attractor_targeted_empty_space", 1f, 1f, weapon.getLocation(), new Vector2f());
			arc.setCoreWidthOverride(thickness * coreWidthMult);
			//arc.setSingleFlickerMode();
		} else {
			Vector2f from = new Vector2f(projectile.getLocation());
			Vector2f to = pickNoTargetDest(projectile, weapon, engine);
			EmpArcEntityAPI arc = engine.spawnEmpArcVisual(from, weapon.getShip(), to, weapon.getShip(), thickness, color, new Color(255, 205, 205,255));
			arc.setCoreWidthOverride(thickness * coreWidthMult);
			//arc.setSingleFlickerMode();
			//Global.getSoundPlayer().playSound("mote_attractor_targeted_empty_space", 1f, 1f, to, new Vector2f());
		}
	}
